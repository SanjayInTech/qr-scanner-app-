...
function showError(message) {
  ...
  async function fetchAndPopulateQCInspectorDropdown() {
    try {
        console.log("Fetching QC Inspector data from API...");
        const response = await fetch(`${backendUrl}/get-data`);
        ...
        const data = await response.json();
        console.log("QC Inspector Data Loaded:", data);

        if (!data.qcInspectors) {
            throw new Error("Invalid API response format for QC Inspectors.");
        }

        populateDropdown("qcInspector", data.qcInspectors, "Enter Inspector Name");

    } catch (error) {
        console.error("Error fetching QC Inspector data:", error);
        alert("Failed to load QC Inspector data. Check your API and Wi-Fi connection.");
    }
}

async function fetchAndPopulateReworkWorkerDropdown() {
    try {
        console.log("Fetching Rework Worker data from API...");
        const response = await fetch(`${backendUrl}/get-data`);
        ...
        const data = await response.json();
        console.log("Rework Worker Data Loaded:", data);

        if (!data.assemblyWorkers) {
            throw new Error("Invalid API response format for Assembly Workers.");
        }

        populateDropdown("reworkerName", data.assemblyWorkers, "Enter Worker Name");

    } catch (error) {
        console.error("Error fetching Rework Worker data:", error);
        alert("Failed to load Rework Worker data. Check your API and Wi-Fi connection.");
    }
}


async function fetchAndPopulateFinalQCInspectorDropdown() {
    try {
        console.log("Fetching Final QC Inspector data from API...");
        const response = await fetch(`${backendUrl}/get-data`);
        ...
        const data = await response.json();
        console.log("Final QC Inspector Data Loaded:", data);

        if (!data.qcInspectors) {
            throw new Error("Invalid API response format for QC Inspectors.");
        }

        populateDropdown("finalQcInspector", data.qcInspectors, "Select Inspector Name");

    } catch (error) {
        console.error("Error fetching Final QC Inspector data:", error);
        alert("Failed to load Final QC Inspector data. Check your API and Wi-Fi connection.");
    }
}


// Fetch and populate Assembly Worker dropdown
async function fetchAndPopulateAssemblyWorkerDropdown() {
    try {
        console.log("Fetching Assembly Worker data from API...");
        const response = await fetch(`${backendUrl}/get-data`);
        ...
        const data = await response.json();
        console.log("Assembly Worker Data Loaded:", data);

        if (!data.assemblyWorkers) {
            throw new Error("Invalid API response format for Assembly Workers.");
        }

        populateDropdown("assemblyWorker", data.assemblyWorkers, "Enter Worker Name");

    } catch (error) {
        console.error("Error fetching Assembly Worker data:", error);
        alert("Failed to load Assembly Worker data. Check your API and Wi-Fi connection.");
    }
}
...
function handleOperationSelection() {
  ...
  function handleQCStatus() {
    ...
    qcFields.innerHTML =
      ...
      `;
      if (status === "start") {
        fetchAndPopulateQCInspectorDropdown();
    }
  }
  ...
  function handleAssemblyStatus() {
    ...
    assemblyFields.innerHTML =
      ...
      if (status === "start") {
        fetchAndPopulateAssemblyWorkerDropdown();
    }
  }
  ...
  function handleReworkStatus() {
    ...
    reworkFields.innerHTML =
      status === "start"
        ...
        <select id="reworkerName">
          <option value="">Enter Worker Name</option>
        </select>
      ...
      if (status === "start") {
        fetchAndPopulateReworkWorkerDropdown();
    }
  }
  ...
  function handleFinalQCStatus() {
    ...
    finalQCFields.innerHTML =
      ...
      if (status === "start") {
        fetchAndPopulateFinalQCInspectorDropdown();
    }
  }
