...
const backendUrl = "http://localhost:3000"; // Update with your actual server IP
...
function handleOperationSelection() {
  ...
  function handleAssemblyStatus() {
    ...
    assemblyFields.innerHTML =
      status === "start"
        ...
        <input type="datetime-local" id="assemblyStart" value="${timestamp}" required>
      `
        : `
        <label for="assemblyEnd">Assembly End Date/Time:</label>
        <input type="datetime-local" id="assemblyEnd" value="${timestamp}" required>
      `;
  }
 ...
 async function handleSubmit(event) {
  ...
  try {
      ...
      const response = await fetch("http://localhost:3000/process-qr", { // ✅ Using IPv4 address
          method: "POST",
